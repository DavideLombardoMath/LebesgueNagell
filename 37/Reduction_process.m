...
/*
Our choice for the parameter \rho
*/
...
procedure TableReductionProcess(D)
	...
	*/
	...
	h := 2^12;
	// r := rExponent(D);
	b1 := 59*h;	// 59 is an upper bound for the values of r that can arise
		
	a1 := 2/h * (rho+1) * Log(eta) ;

	LowerBounda2 := a2min(D);
	gParam := 1/4;	// this is an upper bound for the parameter g. The strange name is due to the fact that we also have a function g.
	...
	*/
	...
	function g(Ktilde, p0)
		...
		*/
		...
		/*
		On the next line, notice the factor 1/2 in (S-1)/(LowerBounda2) * 1/2 * h: this factor comes from the ratio r/p < 1/2
		*/
		Term3 := ( 3/2 + Log( Sqrt(Ktilde*L/a1) + R1 /LowerBounda2 + (S-1)/(LowerBounda2) * 1/2 *h ) - Log( LowerBoundRatio * Ktilde) + Log(p0/2) ) * Ktilde; 
	...
	*/
	function NewEstimate(Ktilde)
		...
		*/
		...
		Notice that we have replace eta^r with eta^59: our previous computations
		have shown that r <= 59 in all cases.
		*/
		delta := Log( 4 * Sqrt(D) * eta^59 ) / LowerBounda2 + c2(D) * (rho+1) / (2 * LowerBounda2) + 10^(-40) + Log(LowerBounda2) / LowerBounda2;
