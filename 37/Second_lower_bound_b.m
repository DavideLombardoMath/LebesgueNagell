SetLogFile("Second_lower_bound_b.out");
/*
...
function TestD(D)
	/*
	For primes up to 157, we test all values of r; for larger primes, we only test r=1
	*/
	for p in PrimesUpTo(150000) do
		Solutions := {};
		...
		if p ge 5 then
			...
			for r in rs do
				LowerBound, SolutionsFound := TestbViaContinuedFractions(r, p, D);
